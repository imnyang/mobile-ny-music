...
function App() {
  ...
  useEffect(() => {
    ...
    if (window.navigator.onLine) {
      fetch(`https://fback.imnyang.xyz//NY64_Cover/list?${Date.now()}`).then(res =>
          res.text().then(data => {
            let pAsta = {};
  ...
  useEffect(() => {
    let interval = setInterval(async () => {
      if (audioContainer.current.paused || String(audioContainer.current.duration) === "NaN") {
        return;
      }
      ...
      if (!window.navigator.onLine) setMusics({ ...savedMusic });
  ...
  return (
    <main>
      ...
      <div id="music">
        ...
        <div>
          <input type="range" min="0" max="100" ref={volume} onChange={() => {audioContainer.current.volume = volume.current.value/100}}  disabled/>
        </div>
      </div>
      <div id="greet">
